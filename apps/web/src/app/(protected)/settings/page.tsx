...
import { Tabs, TabsContent, TabsList, TabsTrigger } from '@web/src/components/ui/tabs'
  ...
  return (
    <Tabs defaultValue="profile" className="w-[400px]">
      <TabsList className="grid w-full grid-cols-2 h-fit gap-2 p-2">
        <TabsTrigger className='p-2 border border-foreground/20' value="profile">{t('profile')}</TabsTrigger>
        <TabsTrigger className='p-2 border border-foreground/20' value="changeEmail">{t('email')}</TabsTrigger>
        <TabsTrigger className='p-2 border border-foreground/20' value="updatePassword">{t('password')}</TabsTrigger>
        <TabsTrigger className='p-2 border border-foreground/20' value="api_keys">{t('api-keys')}</TabsTrigger>
      </TabsList>
